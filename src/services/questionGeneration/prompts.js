...
function createQuestionGenerationPrompt(sessionContext) {
  ...
  const {criticalRules,questionTypeToPrioritize,assessementFocus}=getQuestionRulesByLevel(interviewGoals.targetLevel)
  let systemPrompt='';
  let userPrompt='';
  
  if(interviewGoals.interviewFor == 'core'){

    systemPrompt = `You are an experienced senior technical interviewer conducting a ${interviewGoals.primaryTopic} interview for a ${interviewGoals.targetLevel} candidate applying for ${interviewGoals.targetPosition || 'Technical Role'} positions.
...
- Generate EXACTLY ONE question at a time about ${interviewGoals.primaryTopic} and related concepts
...
CANDIDATE PROFILE:
${interviewGoals.candidateBackground ? `Background: ${interviewGoals.candidateBackground}` : 'Background: Fresh graduate/Entry-level candidate'}
${interviewGoals.specificSkills ? `Key Skills: ${interviewGoals.specificSkills}` : ''}
${interviewGoals.resumeHighlights ? `Resume Highlights: ${interviewGoals.resumeHighlights}` : ''}
...
Generate the most valuable next question for this ${interviewGoals.primaryTopic} interview that:
...
Do not ask any technical or conceptual questions yet—just focus on making the candidate feel comfortable and set a positive, friendly tone for the rest of the interview.`,
    exploration: "Ask probing questions that reveal depth of understanding and practical experience.",
    challenge: "Present realistic scenarios that test problem-solving skills and decision-making ability.",
    closing: "Focus on advanced topics that differentiate strong candidates from average ones."
};
const coreCurrentStage = conversationHistory.length < 2 ? 'opening' : 
                   conversationHistory.length < 6 ? 'exploration' : 
                   conversationHistory.length < 10 ? 'challenge' : 'closing';
